...
When the agent starts, you see output like this:
...
<CardGroup cols={3}>
  ...
  <Card title="Public URL" icon="globe">
    FRP assigns a public subdomain such as `abc123xyz.tunnel.getbindu.com`.
  </Card>
...
<Steps>
  <Step title="Start">
    Your local agent starts on `localhost:3773` with `launch=True` enabled in `bindufy()`.

    That is the only change in the local setup. The rest of the agent continues to run as usual on your machine.
  </Step>

  <Step title="Tunnel">
    The FRP client connects to Bindu's tunnel server and creates a tunnel for the local agent.
  ...
  <Step title="Forward">
    ...
    3. **Public URL Generated** - Random subdomain assigned
...
- **Temporary URLs** - Subdomain changes on each restart unless custom handling exists elsewhere
- **No Persistence** - Tunnel closes when the agent stops

### For Production

- **DO NOT** use tunneling in production

Production workloads should use proper infrastructure instead of a development tunnel.
...
If the tunnel cannot connect, start with the network path and firewall rules before debugging anything in the agent itself.
