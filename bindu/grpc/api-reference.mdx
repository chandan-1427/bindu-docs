...
Server-side streaming variant. **Defined in proto but not implemented** in `GrpcAgentClient`.

<Warning>
Streaming is not yet implemented. See [limitations](/bindu/grpc/limitations) for details.
</Warning>
